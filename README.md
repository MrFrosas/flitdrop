...
- **Mac**: open the `.dmg` for your Mac (`arm64` for Apple Silicon, `x64` for Intel), drag Flitdrop into Applications. The build is ad-hoc signed but not yet notarized by Apple, so on first open macOS may block it. Open it once, then go to **System Settings → Privacy & Security** and click **Open Anyway** (on macOS 14 or older, right-click the app and choose **Open** also works); if it says "damaged", run this once in Terminal: `xattr -cr /Applications/Flitdrop.app`. Until it is notarized, the Mac app does not update itself either: grab the new version from the site. (These steps go away once the app is notarized.)
